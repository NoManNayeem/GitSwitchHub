...
impl GitCredentialHelper {
    ...
    pub fn run(&self) -> Result<(), GitHelperError> {
        ...
        let repo_url = if !url.is_empty() {
            ...
            return Err(GitHelperError::Process("No repository URL found".to_string()));
        };
    ...
    fn show_account_chooser(&self, _repo_url: &str) -> Result<(), GitHelperError> {
        ...
        if accounts.is_empty() {
            return Err(GitHelperError::Process("No GitHub accounts configured".to_string()));
        }
        ...
        if let Ok(token) = self.keychain.get_token(&account.username) {
            ...
            return Err(GitHelperError::Process("No token found for account".to_string()));
        }
